...
class CustomerClientAdapterTest {
    ...
    void notFoundReturnsFalse() {
        ...
        when(client.getCustomerById(eq(99L), anyString())).thenThrow(new FeignException.NotFound("not found", req, null, Collections.emptyMap()));

        boolean valid = adapter.isCustomerValid(99L);
        assertFalse(valid);
    }
    ...
    void getStatus() {
        ...
        resp.setId(1L); resp.setStatus("ACTIVE"); resp.setLoyaltyLevel("GOLD");
        when(client.getCustomerById(eq(1L), anyString())).thenReturn(resp);

        assertEquals("ACTIVE", adapter.getCustomerStatus(1L));
        assertEquals("GOLD", adapter.getCustomerLoyaltyLevel(1L));
    }
}
